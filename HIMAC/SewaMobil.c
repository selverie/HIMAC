...
void printInvoiceKembali(struct Invoice invoice, float jumlah_pembayaran, float denda) {
    printf("-----------------------------");
    printf("\n       Struk Pembayaran");
    printf("\n     Sewa Mobil Pak Fajar");
    printf("\nJl.Magelang Km.12 No.5 Sleman\n\n");
        ...
        switch (menu) {
            ...
            case 2:
                ...
                if (strcmp(nomor_struk, invoice.struk) == 0) {
                    // Menghitung jumlah hari lewat dari batas sewa.
                    int durasi_hari_lewat = 0;
                    sscanf(invoice.batas_sewa, "%d", &durasi_hari_lewat);
                    durasi_hari_lewat -= invoice.durasi_sewa;
                    if (durasi_hari_lewat > 0) {
                        // Jika ada hari lewat, hitung denda.
                        float harga_denda_per_hari;
                        printf("Harga Denda/hari         : RP. ");
                        scanf("%f", &harga_denda_per_hari);
                        denda = harga_denda_per_hari * durasi_hari_lewat;
                    }

                    printf("\n-----------------------------");
                    printf("\n\n       Detail Pembayaran\n");
                    printInvoiceKembali(invoice, jumlah_pembayaran, denda);
                } else {
